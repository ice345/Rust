...
impl Status {
    fn from_str(status: &str) -> Result<String, String> {
        match status {
            "To-Do" => Ok("To-Do".to_string()),
            "In Progress" => Ok("In Progress".to_string()),
...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        ...
        let status = Status::from_str(&status).expect("Only `To-Do`, `In Progress`, and `Done` statuses are allowed");
