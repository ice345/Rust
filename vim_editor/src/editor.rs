use crate::{output::Output, reader::Reader, constants::Mode};
...
impl Editor {
                ...
                match self.reader.read_key()? {
                    ...
                    KeyEvent {
                        ...
                        if self.output.editor_rows.number_of_rows() == 0 {
                            ...
                            self.output.move_cursor(val, self.output.editor_rows.number_of_rows());
                        }
                    }
                    KeyEvent {
                        code: KeyCode::Char('n'),
                ...
                match self.reader.read_key()? {
                    ...
                    KeyEvent {
                        ...
                        if self.command_buffer == "G" {
                            self.output.cursor_controller.cursor_y = if self.output.editor_rows.number_of_rows() == 0 {
                        ...
                        if self.command_buffer.parse::<usize>().is_ok() {
                            ...
                            self.output.cursor_controller.cursor_y = if line != 0 && line <= self.output.editor_rows.number_of_rows() {
                        ...
                        if self.command_buffer == "dd" {
                            self.output.editor_rows.delete_line(self.output.cursor_controller.cursor_y);
                        }
                ...
                match self.reader.read_key()? {
                    ...
                    KeyEvent {
                        ...
                        if !self.command_buffer.is_empty() {
                            ...
                            } else if let Some((row, col)) = self.output.editor_rows.search(&self.command_buffer) {
                                // 光标跳到第一个匹配项
                                self.output.cursor_controller.cursor_y = row;
                                self.output.cursor_controller.cursor_x = col;
                            }
                ...
                match self.reader.read_key()? {
                    ...
                    KeyEvent {
                        ...
                        if self.output.cursor_controller.cursor_x > 0 {
                            ...
                            // 在行首删除，需要将光标移到上一行末尾
                            let prev_row_len = self.output.editor_rows.get_row(
                                self.output.cursor_controller.cursor_y - 1
                            ).len();
                            self.output.cursor_controller.cursor_y -= 1;
    ...
    pub fn run(&mut self) -> crossterm::Result<bool> {
        // 首先刷新屏幕,显示当前状态
        self.output.refresh_screen(&self.mode, &self.command_buffer)?;
        // 处理按键输入
